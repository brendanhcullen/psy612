...
-   One way to estimate linear regression models involves using the `lm()` function, which is part of the pre-loaded {stats} library. There are basically two ingredients we pass to the `lm()` function:

    1.  **The formula:** Specify your regression formula in the general form `y ~ x`.

    2.  **The data:** the dataframe that contains the variables in the formula.

<br>

-   Recall from the previous lab how we wrote out our model, specifying self-rated health as a function of conscientiousness

$$Y_i = b_0 + b_1X_i + e_i$$

$$sr\_health_i = b_0 + b_1consc_i + e_i$$ <br>

-   Fill in the blanks below to specify and estimate the regression model using `lm()`.

<!---LEARNR EX 1-->

<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex1" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex1/" scrolling="no" frameborder="no">
...
> The intercept is the expected value for self-rated health when conscientiousness is 0.

> The slope is the magnitude of the relationship between conscientiousness and self-rated health: for every 1-unit increase in conscientiousness, we expect a 0.49-unit increase in self-rated health.

> *t*-values are from a one-sample *t*-test assessing whether the slope and intercept are significantly different from 0. The *t*-values represent the ratio of signal to noise (i.e. each *b* divided by its standard error).
...
> **What does the 95% CI for the slope of conscientiousness mean in plain English?**

##### Answer

> If we were to repeat this experiment over and over again, sampling from the same population, 95% of the time the slope we calculate would be between 0.25 and 0.73 (i.e. in 19 out of every 20 experiments we'd get a slope in this interval). <br><br> More generally it means that if we carried out random sampling from the population a large number of times, and calculated the 95% CI each time around our coefficients (intercept and slope), then 95% of those intervals can be expected to contain the population parameters. <br><br> In other words, we have good reason to believe the true population parameters ($\beta_0$ and $\beta_1$) fall in this interval because 95% of the time such intervals contain the true population parameters.
...
-   In addition to estimating precision around our coefficients, we can also estimate precision (using standard errors) around each predicted value, $\hat{Y_i}$. These standard errors are generated by `broom::augment()` (and are labeled `.se.fit`).
...
-   If we were to string all of this information together, it would generate a confidence **band** around our regression line. You can generate a confidence band when creating a scatter plot of your data by adding the following layer to your plot: `geom_smooth(method = "lm")`.

<br>

-   Fill in the blanks in the code below to generate a scatter plot with a linear regression line and 95% confidence band.

<!---LEARNR EX 2-->

<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex2" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex2/" scrolling="no" frameborder="no">
...
-   For example, if we know someone's conscientiousness score is `3.5`, we can predict their score for self-rated health according to our model:

$$\hat{health} = 1.6569733 + 0.4904059*3.5 = 3.374$$

<br>

-   The `predict()` function gives us a straightforward way to get the predicted `Y` values from all the `X` values in our dataset.
...
-   The predictions we just generated should look familiar, as we already have gotten this information from `broom::augment()`.
...
-   We can use this information to create and visualize "prediction bands". First we will generate our predicted values (i.e. fitted values) along with a "prediction interval" (lower and upper bound) for each of these values.
...
-   And finally, we'll plot a prediction band on top of the data by adding a `geom_ribbon()`. This prediction band represents, for every value of `X`, the interval that represents the range of values that is likely to contain the value of a single new individual observation. Unlike confidence intervals, prediction intervals predict the spread for individual observations rather than the mean.

-   Fill in the blanks in the code to create a plot of the prediction band for our linear regression model. Pay special attention to `lwr` and `upr` within `pred_plot_data` above. How will you incorporate these variables into the plot?

<!---LEARNR EX 3-->

<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex3" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex3/" scrolling="no" frameborder="no">
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex4" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex4/" scrolling="no" frameborder="no">
...
-   Consider our regression equation in it's general form:
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex5" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex5/" scrolling="no" frameborder="no">
...
-   Convert the `condition` variable to `0's` and `1's`. We will talk more about dummy coding in a future lab.
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex6" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex6/" scrolling="no" frameborder="no">
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex7" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex7/" scrolling="no" frameborder="no">
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex8" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex8/" scrolling="no" frameborder="no">
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex9" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex9/" scrolling="no" frameborder="no">
...
> The intercept is the mean cooperation score for the condition that was coded as 0 (i.e. `public`). The slope is the difference in means between the two conditions.
...
-   Hint: To find the inverse of a matrix, use the `solve()` function. Refer back to the [lab on matrix algebra](https://uopsych.github.io/psy611/labs/lab-3.html){target="_blank"} from PSY611 for a refresher on other matrix operations if needed.

1.  Create the matrix $\mathbf{X}$ & vector $\mathbf{Y}$

<!---LEARNR EX 12-->

<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex12" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex12/" scrolling="no" frameborder="no">
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex13" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex13/" scrolling="no" frameborder="no">
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex14" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex14/" scrolling="no" frameborder="no">
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex10" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex10/" scrolling="no" frameborder="no">
...
<iframe style="margin:0 auto; min-width: 100%;" id="lab-3-glm-ex11" class="interactive" src="https://brendancullen.shinyapps.io/lab-3-glm-ex11/" scrolling="no" frameborder="no">
